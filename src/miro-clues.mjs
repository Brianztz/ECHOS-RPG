...
export async function publishClues(env,storage,nodes,fetcher=fetch){
 ...
 for(const node of publicNodes){
  ...
  if(saved.imageHash!==hash){const metadata={title,geometry:{width:400}};if(!saved.imageId)metadata.position={x:Number(node.x)||0,y:Number(node.y)||0};const mime=image.slice(5,image.indexOf(';')),bytes=Uint8Array.from(atob(image.split(',')[1]),c=>c.charCodeAt(0));const body=new FormData();body.append('resource',new Blob([bytes],{type:mime}),'pista.'+(mime==='image/jpeg'?'jpg':mime.split('/')[1]));body.append('data',JSON.stringify(metadata));const response=await request(saved.imageId?`/images/${encodeURIComponent(saved.imageId)}`:'/images',saved.imageId?'PATCH':'POST',body);const result=await response.json();if(!result.id)throw new Error('Imagem não confirmada pelo Miro.');saved.imageId=result.id;saved.imageHash=hash;await storage.put(key,saved);sent++;}else skipped++;
